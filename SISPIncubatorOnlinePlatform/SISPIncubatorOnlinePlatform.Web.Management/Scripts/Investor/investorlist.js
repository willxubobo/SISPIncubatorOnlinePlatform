...
function GetInvestHtml(data,type) {
    ...
    for (i = 0; i < data.length; i++) {
        ...
        htmlItem += "<div class=\"div_InvestDepTitle\">" + pname + "</div><div class=\"div_InvestDepField\">投资领域：" + data[i].investmentField + "</div>";
        ...
        if (isf == 1) {
            htmlItem += "<div class=\"divConcerned follow\" onclick=\"UnFollowProject('" + data[i].userID + "','" + data[i].followID + "','li" + i + "','" + data[i].followCount + "');return false;\">已关注</div><div>站内信</div>";
        } else {
            htmlItem += "<div onclick=\"FollowProject('" + data[i].userID + "','li" + i + "','" + data[i].followCount + "');return false;\" class=\"nofollow\">关注</div><div style='cursor: pointer;' onclick=\"OpenSendMsgDialog('" + data[i].userID + "','" + data[i].userName + "')\">站内信</div>";
        }
        htmlItem += "</div></li>";
    }
    $('#ul_InvestDep').empty();
