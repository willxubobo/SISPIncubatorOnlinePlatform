...
$(function () {
    ...
    $("#txtRegisteredCapital,#txtDemandForSpace,#txtInitialStaff").bind("keyup", function () {
    ...
    $.ajax(
       {
           ...
           success: function (results) {
               ...
               $(this).alert('提交成功');
               setTimeout(function () {
                   location.href = 'myofficesettledaapply.html';
               }, 2000);
           },
           error: function (err) {
               ErrorResponse(err);
...
function OrderByEndDate(detailArry) {
    ...
    for (var k = detailArry.length - 1; k >= 0 ; k--) {
        ...
        html += " <div class=\"module-title\">" + detailArry[k].startDate + "至" + detailArry[k].endDate + "</div>";
        html += "<div class=\"module-txt\">";
        html += detailArry[k].workAddress;
        if ($.trim(detailArry[k].position) != "") {
            html += "【" + detailArry[k].position + "】";
        }
        html += "</div><div class=\"module-operate\" style='padding-bottom: 10px;'>";
        html += "<a class=\"module-btn right remove-btn ad\" title='删除' onclick='DelItem(this);' ></a>";
        html += "<a class=\"module-btn right edit-btn right20 ae\" title='编辑' onclick='Modify(this)'></a><input  value='" + dataStr + "' class=\"hidValue\" type=\"hidden\" />";
        html += " </div>";
        html += "</div>";
